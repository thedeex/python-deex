...
class Price(GraphenePrice):
    ...
    with the tuple::
        ...
        :param deex.aio.asset.Asset base: Base asset
        :param deex.aio.asset.Asset quote: Quote asset
        :param deex.aio.deex.DeEx blockchain_instance: DeEx instance
        ...
        Way to obtain a proper instance:
            ...
            * ``args`` can be a floating number and ``base`` and ``quote`` being instances of :class:`deex.aio.asset.Asset`
            ...
            * ``args`` being a list of ``[quote, base]`` both being instances of :class:`deex.aio.amount.Amount`
            * ``args`` being a list of ``[quote, base]`` both being instances of ``str`` (``amount symbol``)
            * ``base`` and ``quote`` being instances of :class:`deex.aio.asset.Amount`
    ...
    async def market(self):
        ...
        :returns: Instance of :class:`deex.aio.market.Market` for the
...
class Order(Price):
    """
    This class inherits :class:`deex.aio.price.Price` but has the ``base`` and
