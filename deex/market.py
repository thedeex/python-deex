...
class Market(dict):
    ...
    * ``base`` and ``quote`` are valid assets (according to :class:`deex.asset.Asset`)
    * ``base:quote`` separated with ``:``
    * ``base/quote`` separated with ``/``
    * ``base-quote`` separated with ``-``

    .. note:: Throughout this library, the ``quote`` symbol will be
              presented first (e.g. ``USD:DEEX`` with ``USD`` being the
              quote), while the ``base`` only refers to a secondary asset
              for a trade. This means, if you call
              :func:`deex.market.Market.sell` or
              :func:`deex.market.Market.buy`, you will sell/buy **only
    ...
    def get_string(self, separator=":"):
        """
        Return a formated string that identifies the market, e.g. ``USD:DEEX``
    ...
    def ticker(self):
        ...
        * ``core_exchange_rate``: Core exchange rate for payment of fee in non-DEEX asset
    ...
    def orderbook(self, limit=25):
        ...
        .. note:: Each bid is an instance of
            class:`deex.price.Order` and thus carries the keys
    ...
    def accountopenorders(self, account=None):
        ...
        :param deex.account.Account account: Account name or instance of Account to show orders for in this market
        """
        if not account:
            if "default_account" in self.blockchain.config:
                account = self.blockchain.config["default_account"]
