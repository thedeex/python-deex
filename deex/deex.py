...
from deexapi.deexnoderpc import DeExNodeRPC
from deexbase import operations
from deexbase.account import PublicKey
from deexbase.asset_permissions import asset_permissions, toint
...
class DeEx(AbstractGrapheneChain):
    """
    Connect to the DeEx network.
    ...
    * **Wallet Database**: Here, the deexlibs load the keys from the
      locally stored wallet SQLite database (see ``storage.py``).
      To use this mode, simply call ``DeEx()`` without the
      ``keys`` parameter
    * **Providing Keys**: Here, you can provide the keys for
      your accounts manually. All you need to do is add the wif
      keys for the accounts you want to use as a simple array
      using the ``keys`` parameter to ``DeEx()``.
    * **Force keys**: This more is for advanced users and
    ...
    def create_account(
        ...
        **kwargs
        ...
        """
        Create new account on DeEx.

        The brainkey/password can be used to recover all generated keys
        (see `deexbase.account` for more details.
        ...
        from deexbase.account import PasswordKey, PublicKey
    ...
    def set_proxy(self, proxy_account, account=None, **kwargs):
        ...
        :param deex.account.Account proxy_account: Account to be
                proxied
        :param str account: (optional) the account to allow access
            to (defaults to ``default_account``)
        """
    ...
    def vesting_balance_withdraw(self, vesting_id, amount=None, account=None, **kwargs):
        ...
        :param deex.amount.Amount Amount: to withdraw ("all" if not
            provided")
        :param str account: (optional) the account to allow access
            to (defaults to ``default_account``)
        """
    ...
    def publish_price_feed(
        ...
        :param deex.price.Price settlement_price: Price for settlement
        :param deex.price.Price cer: Core exchange Rate (default
            ``settlement_price + 5%``)
        ...
        ), "settlement_price needs to be instance of `deex.price.Price`!"
        assert cer is None or isinstance(
            cer, Price
        ), "cer needs to be instance of `deex.price.Price`!"
        if not account:
            if "default_account" in self.config:
                account = self.config["default_account"]
        if not account:
    ...
    def update_cer(self, symbol, cer, account=None):
        ...
        :param str symbol: Symbol of the asset to publish feed for
        :param deex.price.Price cer: Core exchange Rate
        :param str account: (optional) the account to allow access
            to (defaults to ``default_account``)
        """
        assert isinstance(
    ...
    def reserve(self, amount, account=None, **kwargs):
        ...
        :param deex.amount.Amount amount: The amount to be burned.
        :param str account: (optional) the account to allow access
            to (defaults to ``default_account``)
        """
    ...
    def create_worker(
        ...
        **Required**
        ...
        :param deex.amount.Amount daily_pay: The amount to be paid
            daily
        ...
        **Optional**
        ...
        :param str account: (optional) the account to allow access
            to (defaults to ``default_account``)
        """
        from deexbase.transactions import timeformat
