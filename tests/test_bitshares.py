...
from deex.amount import Amount
from deexbase.account import PrivateKey
from deexbase.asset_permissions import todict
...
class Testcases(unittest.TestCase):
    ...
    def test_set_default_account(self):
        deex.set_default_account("init0")
    ...
    def test_finalizeOps(self):
        tx1 = deex.new_tx()
        tx2 = deex.new_tx()
        deex.transfer("init1", 1, "DX", append_to=tx1)
        deex.transfer("init1", 2, "DX", append_to=tx2)
        deex.transfer("init1", 3, "DX", append_to=tx1)
        tx1 = tx1.json()
        tx2 = tx2.json()
        ops1 = tx1["operations"]
        ops2 = tx2["operations"]
    ...
    def test_transfer(self):
        tx = deex.transfer("1.2.101", 1.33, "DX", memo="Foobar", account="init0")
    ...
    def test_create_account(self):
        ...
        op = tx["operations"][0][1]
        role = "active"
        self.assertIn(format(key4.pubkey, "DX"), [x[0] for x in op[role]["key_auths"]])
        self.assertIn(format(key4.pubkey, "DX"), [x[0] for x in op[role]["key_auths"]])
    ...
    def test_weight_threshold(self):
        ...
        auth = {
            ...
            "key_auths": [
                ...
                ["DX7GM9YXcsoAJAgKbqW2oVj7bnNXFNL4pk9NugqKWPmuhoEDbkDv", 1],
            ],
            "weight_threshold": 3,
        }  # threshold fine
        deex._test_weights_treshold(auth)
        auth = {
            "account_auths": [["1.2.0", "1"]],
            "extensions": [],
            "key_auths": [
    ...
    def test_disallow(self):
        ...
        with self.assertRaisesRegex(ValueError, "Cannot have threshold of 0"):
            deex.disallow(
                "DX6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
                weight=1,
                threshold=1,
                permission="owner",
            )
        deex.disallow(
            "DX5i8bEmtnN4fP4jAsBe17z9CCuQcHLkRyTuRZXYZeN2kVCL1sXa",
            weight=1,
            threshold=1,
            permission="active",
        )
