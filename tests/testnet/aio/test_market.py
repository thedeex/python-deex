...
from deex.aio.price import Price, Order
...
@pytest.mark.asyncio
async def test_core_quote_market(deex, assets, bitasset):
    market = await Market(
        "{}:USD".format(bitasset.symbol), blockchain_instance=deex
    )
    await market.core_quote_market()


@pytest.mark.asyncio
async def test_core_base_market(deex, assets, bitasset):
    market = await Market(
        "USD:{}".format(bitasset.symbol), blockchain_instance=deex
    )
