...
from deex.aio.price import Price, PriceFeed
...
@pytest.fixture
async def asset(deex):
    return await Asset("TEST", blockchain_instance=deex)


@pytest.fixture
async def testasset(deex, create_asset, unused_asset):
    asset = await unused_asset()
    log.info("Creating asset {}".format(asset))
    await create_asset(asset, 4)
    return await Asset(asset, blockchain_instance=deex)


@pytest.fixture(scope="session")
async def bitasset_local(deex, base_bitasset, default_account):
