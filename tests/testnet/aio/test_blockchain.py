...
from deex.aio.blockchain import Blockchain
...
async def chain(deex):
    return await Blockchain(mode="head", blockchain_instance=deex)
...
@pytest.mark.asyncio
async def test_awaitTxConfirmation(deex, chain, default_account):
    trx = await deex.transfer(
