...
from deex.aio.price import Price, Order, FilledOrder
...
@pytest.mark.asyncio
async def test_price_init(deex, assets):
...
@pytest.mark.asyncio
async def test_order_repr(deex, default_account, market):
    # Load from id
    await asyncio.sleep(1.1)
    ...
    result = await deex.rpc.get_objects([tx["orderid"]])
...
async def test_order_init_no_shared_instance(
    ...
    deex = not_shared_instance

    # Load from id
    await asyncio.sleep(1.1)
    tx = await market.buy(1, 1, account=default_account, returnOrderId="head")
    order = await Order(tx["orderid"], blockchain_instance=deex)
    assert "id" in order
    log.info("Order from id: {}".format(order))

    # Load from raw object 1.7.x
    result = await deex.rpc.get_objects([tx["orderid"]])
    order = await Order(result[0], blockchain_instance=deex)
    assert "id" in order
    log.info("Order from object 1.7.x: {}".format(order))

    # Load from an operation
    trx = await market.buy(1, 1, account=default_account)
    order = await Order(trx["operations"][0][1], blockchain_instance=deex)
    log.info("Order from an operation: {}".format(order))
